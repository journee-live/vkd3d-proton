...
dxgi_vk_swap_chain_factory_AddRef(IDXGIVkSwapChainFactory *iface) {
  struct dxgi_vk_swap_chain_factory *chain =
      impl_from_IDXGIVkSwapChainFactory(iface);
  TRACE("iface %p\n", iface);
  return ID3D12CommandQueue_AddRef(&chain->queue->ID3D12CommandQueue_iface);
}

static ULONG STDMETHODCALLTYPE
dxgi_vk_swap_chain_factory_Release(IDXGIVkSwapChainFactory *iface) {
  struct dxgi_vk_swap_chain_factory *chain =
      impl_from_IDXGIVkSwapChainFactory(iface);
  TRACE("iface %p\n", iface);
...
static HRESULT STDMETHODCALLTYPE dxgi_vk_swap_chain_factory_QueryInterface(
    IDXGIVkSwapChainFactory *iface, REFIID riid, void **object) {
  struct dxgi_vk_swap_chain_factory *chain =
      impl_from_IDXGIVkSwapChainFactory(iface);
  TRACE("iface %p\n", iface);
  return ID3D12CommandQueue_QueryInterface(
      &chain->queue->ID3D12CommandQueue_iface, riid, object);
}
...
struct dxgi_vk_swap_chain {
  ...
  struct {
    /* When resizing user buffers, we need to make sure all pending blits have
     * completed on GPU. */
    VkSemaphore vk_blit_semaphore;
    uint64_t blit_count;

    /* PresentID or frame latency fence is used depending on features and if
     * we're really presenting on-screen. */
     ...
     * We just care about equality check so we can atomically check if all
     * outstanding present events have completed on CPU timeline. This is used
     * to implement occlusion check. */
    uint32_t present_count;

    /* For blits. Use simple VkFences since we have to use binary semaphores
     * with WSI release anyways. We don't need to wait on these fences on main
     * thread. */
    ...
    /* Since we're presenting in a thread, there's no particular reason to use
     * WSI acquire semaphores. Removes a lot of edge cases. */
  ...
  struct dxgi_vk_swap_chain_present_request request,
      request_ring[DXGI_MAX_SWAP_CHAIN_BUFFERS];
  ...
  struct {
    ...
    pthread_mutex_t lock;
    pthread_cond_t cond;
    struct streamer_thread_wait_entry *wait_queue;
    size_t wait_queue_size;
    size_t wait_queue_count;
    bool active;
  } streamer_thread;
...
static void dxgi_vk_swap_chain_drain_queue(struct dxgi_vk_swap_chain *chain) {
   ...
   * This happens on the fence waiter queues, so it's not enough to call
   * vkQueueWaitIdle to be 100% sure. The fence waiter thread processes requests
   * in-order, so if we observe that an EVENT has been signalled, we know all
   * pending semaphore signals have happened as well. */

  /* This is safe since we have drained all outstanding CPU work on the queue
   * after calling vkd3d_acquire_vk_queue(). */
  chain->present.frame_latency_count += 1;
  d3d12_command_queue_signal_inline(chain->queue,
                                    chain->present.frame_latency_fence,
                                    chain->present.frame_latency_count);
  d3d12_fence_set_event_on_completion(
      impl_from_ID3D12Fence1(chain->present.frame_latency_fence),
      chain->present.frame_latency_count, NULL);
}

static void
dxgi_vk_swap_chain_drain_blit_semaphore(struct dxgi_vk_swap_chain *chain,
                                        uint64_t value) {
  const struct vkd3d_vk_device_procs *vk_procs =
      &chain->queue->device->vk_procs;
  ...
  vr = VK_CALL(vkWaitSemaphores(chain->queue->device->vk_device, &wait_info,
                                UINT64_MAX));
  if (vr)
...
static void
dxgi_vk_swap_chain_drain_user_images(struct dxgi_vk_swap_chain *chain) {
...
static void dxgi_vk_swap_chain_push_present_id(struct dxgi_vk_swap_chain *chain,
  ...
  vkd3d_array_reserve((void **)&chain->wait_thread.wait_queue,
                      &chain->wait_thread.wait_queue_size,
                      chain->wait_thread.wait_queue_count + 1,
                      sizeof(*chain->wait_thread.wait_queue));
...
static void dxgi_vk_swap_chain_wait_and_reset_acquire_fence(
...
static void dxgi_vk_swap_chain_cleanup(struct dxgi_vk_swap_chain *chain) {
  ...
  VK_CALL(vkDestroySemaphore(chain->queue->device->vk_device,
                             chain->present.vk_blit_semaphore, NULL));
  VK_CALL(vkDestroyCommandPool(chain->queue->device->vk_device,
                               chain->present.vk_blit_command_pool, NULL));
  for (i = 0; i < ARRAY_SIZE(chain->present.vk_release_semaphores); i++)
    VK_CALL(vkDestroySemaphore(chain->queue->device->vk_device,
                               chain->present.vk_release_semaphores[i], NULL));
  for (i = 0; i < ARRAY_SIZE(chain->present.vk_backbuffer_image_views); i++)
    VK_CALL(vkDestroyImageView(chain->queue->device->vk_device,
                               chain->present.vk_backbuffer_image_views[i],
                               NULL));

  dxgi_vk_swap_chain_wait_and_reset_acquire_fence(chain);
  VK_CALL(vkDestroyFence(chain->queue->device->vk_device,
                         chain->present.vk_acquire_fence, NULL));
  for (i = 0; i < ARRAY_SIZE(chain->present.vk_blit_fences); i++)
    VK_CALL(vkDestroyFence(chain->queue->device->vk_device,
                           chain->present.vk_blit_fences[i], NULL));

  VK_CALL(vkDestroySwapchainKHR(chain->queue->device->vk_device,
                                chain->present.vk_swapchain, NULL));

  for (i = 0; i < ARRAY_SIZE(chain->user.backbuffers); i++) {
    if (chain->user.backbuffers[i])
      vkd3d_resource_decref(
          (ID3D12Resource *)&chain->user.backbuffers[i]->ID3D12Resource_iface);
    VK_CALL(vkDestroyImageView(chain->queue->device->vk_device,
                               chain->user.vk_image_views[i], NULL));
  }
...
static inline struct dxgi_vk_swap_chain *
impl_from_IDXGIVkSwapChain(IDXGIVkSwapChain *iface) {
  return CONTAINING_RECORD(iface, struct dxgi_vk_swap_chain,
                           IDXGIVkSwapChain_iface);
}

static ULONG STDMETHODCALLTYPE
dxgi_vk_swap_chain_AddRef(IDXGIVkSwapChain *iface) {
...
static ULONG STDMETHODCALLTYPE
dxgi_vk_swap_chain_Release(IDXGIVkSwapChain *iface) {
  struct dxgi_vk_swap_chain *chain = impl_from_IDXGIVkSwapChain(iface);
...
static HRESULT STDMETHODCALLTYPE dxgi_vk_swap_chain_QueryInterface(
    IDXGIVkSwapChain *iface, REFIID riid, void **object) {
  struct dxgi_vk_swap_chain *chain = impl_from_IDXGIVkSwapChain(iface);
  TRACE("iface %p\n", iface);
...
static HRESULT STDMETHODCALLTYPE dxgi_vk_swap_chain_GetDesc(
    IDXGIVkSwapChain *iface, DXGI_SWAP_CHAIN_DESC1 *pDesc) {
  struct dxgi_vk_swap_chain *chain = impl_from_IDXGIVkSwapChain(iface);
...
static HRESULT STDMETHODCALLTYPE dxgi_vk_swap_chain_GetAdapter(
    IDXGIVkSwapChain *iface, REFIID riid, void **object) {
  struct dxgi_vk_swap_chain *chain = impl_from_IDXGIVkSwapChain(iface);
  TRACE("iface %p\n", iface);
...
static HRESULT STDMETHODCALLTYPE dxgi_vk_swap_chain_GetDevice(
    IDXGIVkSwapChain *iface, REFIID riid, void **object) {
  struct dxgi_vk_swap_chain *chain = impl_from_IDXGIVkSwapChain(iface);
  TRACE("iface %p\n", iface);
  return ID3D12Device12_QueryInterface(
      &chain->queue->device->ID3D12Device_iface, riid, object);
}

static HRESULT STDMETHODCALLTYPE dxgi_vk_swap_chain_GetImage(
    IDXGIVkSwapChain *iface, UINT BufferId, REFIID riid, void **object) {
  struct dxgi_vk_swap_chain *chain = impl_from_IDXGIVkSwapChain(iface);
  TRACE("iface %p, BufferId %u.\n", iface, BufferId);
  if (BufferId >= chain->desc.BufferCount)
    return E_INVALIDARG;
  return ID3D12Resource2_QueryInterface(
      &chain->user.backbuffers[BufferId]->ID3D12Resource_iface, riid, object);
}

static UINT STDMETHODCALLTYPE
dxgi_vk_swap_chain_GetImageIndex(IDXGIVkSwapChain *iface) {
  struct dxgi_vk_swap_chain *chain = impl_from_IDXGIVkSwapChain(iface);
  TRACE("iface %p.\n", iface);
...
static UINT STDMETHODCALLTYPE
dxgi_vk_swap_chain_GetFrameLatency(IDXGIVkSwapChain *iface) {
  struct dxgi_vk_swap_chain *chain = impl_from_IDXGIVkSwapChain(iface);
  TRACE("iface %p.\n", iface);
  return chain->frame_latency;
}

static HANDLE STDMETHODCALLTYPE
dxgi_vk_swap_chain_GetFrameLatencyEvent(IDXGIVkSwapChain *iface) {
...
#ifdef _WIN32
  /* Based on observation, this handle can be waited on, but ReleaseSemaphore()
   * is not allowed. Verified that NtQueryObject returns 0x100000 access mask
   * (SYNCHRONIZE only). */
  if (!DuplicateHandle(
          GetCurrentProcess(), swapchain->frame_latency_event.handle,
          GetCurrentProcess(), &duplicated_handle, SYNCHRONIZE, FALSE, 0)) {
...
static HRESULT
dxgi_vk_swap_chain_allocate_user_buffer(struct dxgi_vk_swap_chain *chain,
                                        const DXGI_SWAP_CHAIN_DESC1 *pDesc,
                                        struct d3d12_resource **ppResource) {
  ...
  return d3d12_resource_create_committed(
      device, &resource_desc, &heap_props, D3D12_HEAP_FLAG_NONE,
      D3D12_RESOURCE_STATE_PRESENT, NULL, 0, NULL, NULL, ppResource);
}

static HRESULT
dxgi_vk_swap_chain_reallocate_user_buffers(struct dxgi_vk_swap_chain *chain) {
  const struct vkd3d_vk_device_procs *vk_procs =
      &chain->queue->device->vk_procs;
  ...
  for (i = 0; i < chain->desc.BufferCount; i++) {
    if (FAILED(hr = dxgi_vk_swap_chain_allocate_user_buffer(
                   chain, &chain->desc, &chain->user.backbuffers[i])))
      goto err;
    ...
    vr = VK_CALL(vkCreateImageView(chain->queue->device->vk_device, &view_info,
                                   NULL, &chain->user.vk_image_views[i]));
    if (vr < 0) {
  ...
  for (i = 0; i < DXGI_MAX_SWAP_CHAIN_BUFFERS; i++)
    if (old_resources[i])
      vkd3d_resource_decref(
          (ID3D12Resource *)&old_resources[i]->ID3D12Resource_iface);
...
static HRESULT STDMETHODCALLTYPE dxgi_vk_swap_chain_ChangeProperties(
    IDXGIVkSwapChain *iface, const DXGI_SWAP_CHAIN_DESC1 *pDesc,
  ...
  TRACE("iface %p, pDesc %p, pNodeMasks %p, ppPresentQueues %p!\n", iface,
        pDesc, pNodeMasks, ppPresentQueues);
  ...
  /* Waits for any outstanding present event to complete, including the work it
   * takes to blit to screen. */
  ...
  if (chain->user.index >= chain->desc.BufferCount) {
     ...
     * It is unclear if we're allowed to always reset, but employ principle of
     * least surprise. */
    chain->user.index = 0;
  }
...
static HRESULT STDMETHODCALLTYPE dxgi_vk_swap_chain_SetPresentRegion(
    IDXGIVkSwapChain *iface, const RECT *pRegion) {
  FIXME("iface %p, pRegion %p stub!\n", iface, pRegion);
  return E_NOTIMPL;
}

static HRESULT STDMETHODCALLTYPE dxgi_vk_swap_chain_SetGammaControl(
    IDXGIVkSwapChain *iface, UINT NumControlPoints,
    const DXGI_RGB *pControlPoints) {
  FIXME("iface %p, NumControlPoints %u, pControlPoints %p stub!\n", iface,
        NumControlPoints, pControlPoints);
  return E_NOTIMPL;
}

static HRESULT STDMETHODCALLTYPE
dxgi_vk_swap_chain_SetFrameLatency(IDXGIVkSwapChain *iface, UINT MaxLatency) {
  struct dxgi_vk_swap_chain *chain = impl_from_IDXGIVkSwapChain(iface);
  ...
  if (!(chain->desc.Flags &
    ...
    WARN("DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT not set for swap "
         "chain %p.\n",
         iface);
    return DXGI_ERROR_INVALID_CALL;
  }
  ...
  if (MaxLatency > chain->frame_latency)
    vkd3d_native_sync_handle_release(chain->frame_latency_event,
                                     MaxLatency - chain->frame_latency);
...
static float convert_level(UINT16 dxgi_level) { return (float)dxgi_level; }

static VkHdrMetadataEXT
convert_hdr_metadata_hdr10(DXGI_HDR_METADATA_HDR10 *dxgi_metadata) {
  VkHdrMetadataEXT vulkan_metadata = {VK_STRUCTURE_TYPE_HDR_METADATA_EXT};
  vulkan_metadata.displayPrimaryRed =
      convert_xy_color(dxgi_metadata->RedPrimary);
  vulkan_metadata.displayPrimaryGreen =
      convert_xy_color(dxgi_metadata->GreenPrimary);
  vulkan_metadata.displayPrimaryBlue =
      convert_xy_color(dxgi_metadata->BluePrimary);
  vulkan_metadata.whitePoint = convert_xy_color(dxgi_metadata->WhitePoint);
  vulkan_metadata.maxLuminance =
      convert_max_luminance(dxgi_metadata->MaxMasteringLuminance);
  vulkan_metadata.minLuminance =
      convert_min_luminance(dxgi_metadata->MinMasteringLuminance);
  vulkan_metadata.maxContentLightLevel =
      convert_level(dxgi_metadata->MaxContentLightLevel);
  vulkan_metadata.maxFrameAverageLightLevel =
      convert_level(dxgi_metadata->MaxFrameAverageLightLevel);
  return vulkan_metadata;
}

static void
dxgi_vk_swap_chain_set_hdr_metadata(struct dxgi_vk_swap_chain *chain) {
  const struct vkd3d_vk_device_procs *vk_procs =
      &chain->queue->device->vk_procs;
  ...
  hdr_metadata =
      convert_hdr_metadata_hdr10(&chain->request.dxgi_hdr_metadata.HDR10);
  VK_CALL(vkSetHdrMetadataEXT(chain->queue->device->vk_device, 1,
                              &chain->present.vk_swapchain, &hdr_metadata));
}

static bool
dxgi_vk_swap_chain_present_task_is_idle(struct dxgi_vk_swap_chain *chain) {
  uint32_t presented_count = vkd3d_atomic_uint32_load_explicit(
      &chain->present.present_count, vkd3d_memory_order_acquire);
...
static bool dxgi_vk_swap_chain_is_occluded(struct dxgi_vk_swap_chain *chain) {
  ...
  VK_CALL(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
      vk_physical_device, chain->vk_surface, &surface_caps));
  /* Win32 jank, when these are 0 we cannot create a swapchain. */
  return surface_caps.maxImageExtent.width == 0 ||
         surface_caps.maxImageExtent.height == 0;
}

static bool
dxgi_vk_swap_chain_present_is_occluded(struct dxgi_vk_swap_chain *chain) {
  if (dxgi_vk_swap_chain_present_task_is_idle(chain)) {
    ...
    /* If presentation requests are pending it is not safe to access the surface
     * directly without adding tons of locks everywhere, so rely on observed
     * behavior from presentation thread. */
    return vkd3d_atomic_uint32_load_explicit(&chain->present.is_occlusion_state,
                                             vkd3d_memory_order_relaxed) != 0;
  }
}

static void dxgi_vk_swap_chain_present_callback(void *chain);

static VkImage create_vulkan_image(struct d3d12_device *device,
  ...
  vr = VK_CALL(vkCreateImage(vk_device, &image_info, NULL, &vk_image));
...
static unsigned int
select_vulkan_memory_type(struct d3d12_device *device,
  ...
  for (i = 0; i < memory_info.memoryTypeCount; ++i) {
    if (!(memory_type_mask & (1u << i)))
      continue;
    if ((memory_info.memoryTypes[i].propertyFlags & required_flags) ==
        required_flags)
      return i;
  }
...
allocate_vulkan_device_memory(struct d3d12_device *device,
                              VkMemoryPropertyFlags required_flags,
  ...
  allocate_info.pNext = &memory_export_info;
  allocate_info.allocationSize = memory_requirements->size;
  allocate_info.memoryTypeIndex = select_vulkan_memory_type(
      device, memory_requirements->memoryTypeBits, required_flags);

  memory_export_info = (struct VkExportMemoryAllocateInfo){
      .sType = VK_STRUCTURE_TYPE_EXPORT_MEMORY_ALLOCATE_INFO,
      .pNext = NULL,
      .handleTypes = VK_EXTERNAL_MEMORY_HANDLE_TYPE_DMA_BUF_BIT_EXT,
  };

  vr = VK_CALL(vkAllocateMemory(vk_device, &allocate_info, NULL, &vk_memory));

  return vk_memory;
}

static VkDeviceMemory
allocate_vulkan_image_memory(struct d3d12_device *device,
                             VkMemoryPropertyFlags required_flags,
                             VkImage vk_image) {
  const struct vkd3d_vk_device_procs *vk_procs = &device->vk_procs;
  ...
  vk_memory = allocate_vulkan_device_memory(device, required_flags,
                                            &memory_requirements);
  ...
  vr = VK_CALL(vkBindImageMemory2(vk_device, 1, &bind_info));
...
static HRESULT STDMETHODCALLTYPE dxgi_vk_swap_chain_Present(
    IDXGIVkSwapChain *iface, UINT SyncInterval, UINT PresentFlags,
  ...
  // TODO: Do the copying here copy copy to streamer texture (lukas)

  // struct d3d12_resource *back_buffer;
  // struct d3d12_device *device;

  // static VkImage vk_image = NULL;
  // static VkDeviceMemory vk_memory;

  // // Get fd and shit. Send via shared memory?

  // back_buffer = chain->user.backbuffers[chain->user.index];
  // device = back_buffer->device;

  // // VkDevice vk_device;
  // INFO("Streamer says hello.\n");

  // if(!vk_image) {
  //     INFO("Streamer allocating VK imge.\n");
  //     vk_image = create_vulkan_image(device, 1920, 1080,
   ...
   * Otherwise, we end up in a floating state where our waits and thread signals
   * might not stay in sync anymore. */
  if (chain->outstanding_present_request) {
    vkd3d_native_sync_handle_acquire(chain->present_request_done_event);
    chain->outstanding_present_request = false;
  }
  ...
  /* The present iteration on present thread has a similar counter and it will
   * pick up the request from the ring. */
  chain->user.present_count += 1;
  request = &chain->request_ring[chain->user.present_count %
                                 ARRAY_SIZE(chain->request_ring)];

  request->swap_interval = SyncInterval;
  request->dxgi_format =
      chain->user.backbuffers[chain->user.index]->desc.Format;
  request->user_index = chain->user.index;
  /* If we don't have wait thread, BufferCount needs to be tweaked to control
   * latency. Some games that create BufferCount = 3 swapchains expect us to
   * absorb a lot of latency or we start starving. If we have waiter thread,
   * we'll block elsewhere. */
  request->target_min_image_count =
      chain->wait_thread.active ? 0 : chain->desc.BufferCount + 1;
  ...
  d3d12_command_queue_enqueue_callback(
      chain->queue, dxgi_vk_swap_chain_present_callback, chain);

  chain->user.index = (chain->user.index + 1) % chain->desc.BufferCount;

  /* Relevant if application does not use latency fence, or we force a lower
   * latency through VKD3D_SWAPCHAIN_FRAME_LATENCY overrides. */
  if (vkd3d_native_sync_handle_is_valid(chain->frame_latency_event_internal))
    vkd3d_native_sync_handle_acquire(chain->frame_latency_event_internal);

  if (vkd3d_native_sync_handle_is_valid(chain->present_request_done_event)) {
    /* For non-present wait path where we are trying to simulate
     * KHR_present_wait in a poor man's way. Block here until we have processed
     * the present and acquired the next image. This is equivalent to a frame
     * latency of swapchain.imageCount - 1. (Usually 2). To combat deadlocks, we
     * can add a small timeout. Failing the timeout is not severe. It will
     * manifest as stutter, but it is better than spurious deadlock. When
     * KHR_present_wait is supported, this path is never taken. If we're heavily
     * GPU bound, we will generally end up blocking on GPU-completion fences in
     * game code instead. When we are present bound, we will generally always
     * render at > 15 Hz. */
    if (!vkd3d_native_sync_handle_acquire_timeout(
            chain->present_request_done_event, 80)) {
      WARN("Detected excessively slow Present() processing. Potential causes: "
           "resize, wait-before-signal.\n");
      /* Remember to wait for this next present. */
      chain->outstanding_present_request = true;
    }
  }

  /* For latency debug purposes. Consider a frame to begin when we return from
   * Present() with the next user index set. This isn't necessarily correct if
   * the application does WaitSingleObject() on the latency right after this
   * call. That call can take up a frame, so the real latency will be lower than
   * the one reported. Otherwise, the estimate should match up with the internal
   * latency fence. */
...
convert_color_space(DXGI_COLOR_SPACE_TYPE dxgi_color_space);
static bool dxgi_vk_swap_chain_update_formats(struct dxgi_vk_swap_chain *chain);

static bool
dxgi_vk_swap_chain_supports_color_space(struct dxgi_vk_swap_chain *chain,
  ...
  if (dxgi_vk_swap_chain_present_task_is_idle(chain)) {
    ...
    if ((vr = VK_CALL(vkGetPhysicalDeviceSurfaceFormatsKHR(
             vk_physical_device, chain->vk_surface, &format_count, NULL))) <
        0) {
    ...
    if ((vr = VK_CALL(vkGetPhysicalDeviceSurfaceFormatsKHR(
             vk_physical_device, chain->vk_surface, &format_count, formats))) <
        0) {
      ERR("Failed to query surface formats, vr %d.\n", vr);
      goto out;
    }
...
static UINT STDMETHODCALLTYPE dxgi_vk_swap_chain_CheckColorSpaceSupport(
    IDXGIVkSwapChain *iface, DXGI_COLOR_SPACE_TYPE ColorSpace) {
  struct dxgi_vk_swap_chain *chain = impl_from_IDXGIVkSwapChain(iface);
...
static HRESULT STDMETHODCALLTYPE dxgi_vk_swap_chain_SetColorSpace(
    IDXGIVkSwapChain *iface, DXGI_COLOR_SPACE_TYPE ColorSpace) {
  struct dxgi_vk_swap_chain *chain = impl_from_IDXGIVkSwapChain(iface);
...
static HRESULT STDMETHODCALLTYPE dxgi_vk_swap_chain_SetHDRMetaData(
    IDXGIVkSwapChain *iface, const DXGI_VK_HDR_METADATA *pMetaData) {
  struct dxgi_vk_swap_chain *chain = impl_from_IDXGIVkSwapChain(iface);
...
static bool
dxgi_vk_swap_chain_update_formats(struct dxgi_vk_swap_chain *chain) {
  const struct vkd3d_vk_device_procs *vk_procs =
      &chain->queue->device->vk_procs;
  VkPhysicalDevice vk_physical_device =
      chain->queue->device->vk_physical_device;
  ...
  if ((vr = VK_CALL(vkGetPhysicalDeviceSurfaceFormatsKHR(
           vk_physical_device, chain->vk_surface,
           &chain->properties.format_count, NULL))) < 0) {
    ERR("Failed to query surface formats.\n");
    return false;
  }

  if (!vkd3d_array_reserve(
          (void **)&chain->properties.formats, &chain->properties.formats_size,
...
static HRESULT
dxgi_vk_swap_chain_create_surface(struct dxgi_vk_swap_chain *chain,
                                  IDXGIVkSurfaceFactory *pFactory) {
  const struct vkd3d_vk_device_procs *vk_procs =
      &chain->queue->device->vk_procs;
  VkPhysicalDevice vk_physical_device;
  ...
  vr = IDXGIVkSurfaceFactory_CreateSurface(
      pFactory, vk_instance, vk_physical_device, &chain->vk_surface);
  ...
  vr = VK_CALL(vkGetPhysicalDeviceSurfaceSupportKHR(
      vk_physical_device, chain->queue->vkd3d_queue->vk_family_index,
...
static HRESULT
dxgi_vk_swap_chain_init_sync_objects(struct dxgi_vk_swap_chain *chain) {
  const struct vkd3d_vk_device_procs *vk_procs =
      &chain->queue->device->vk_procs;
  ...
  if (FAILED(hr = ID3D12Device12_CreateFence(
                 &chain->queue->device->ID3D12Device_iface, 0,
                 D3D12_FENCE_FLAG_NONE, &IID_ID3D12Fence1,
                 (void **)&chain->present.frame_latency_fence))) {
  ...
  if (chain->desc.Flags & DXGI_SWAP_CHAIN_FLAG_FRAME_LATENCY_WAITABLE_OBJECT) {
    ...
    if (FAILED(hr = vkd3d_native_sync_handle_create(
                   chain->frame_latency,
                   VKD3D_NATIVE_SYNC_HANDLE_TYPE_SEMAPHORE,
                   &chain->frame_latency_event))) {
      WARN("Failed to create frame latency semaphore, hr %#x.\n", hr);
      return hr;
    }
  } else {
#define DEFAULT_FRAME_LATENCY 3
    /* 3 frames is the default in the API, so that's what we should report to
     * application if asks. */
  ...
  if (chain->queue->device->device_info.present_wait_features.presentWait) {
     ...
     * If we don't have present wait, we will sync with
     * present_request_done_event (below) instead. Adding more sync against
     * internal blit fences is meaningless and can cause weird pumping issues in
     * some cases since we're simultaneously syncing against two different
     * timelines. */
    ...
    if (chain->frame_latency_internal >= 1 &&
        chain->frame_latency_internal < DXGI_MAX_SWAP_CHAIN_BUFFERS) {
      ...
      if (FAILED(hr = vkd3d_native_sync_handle_create(
                     chain->frame_latency_internal - 1,
                     VKD3D_NATIVE_SYNC_HANDLE_TYPE_SEMAPHORE,
                     &chain->frame_latency_event_internal))) {
  ...
  if (!chain->queue->device->device_info.present_wait_features.presentWait &&
      FAILED(hr = vkd3d_native_sync_handle_create(
                 0, VKD3D_NATIVE_SYNC_HANDLE_TYPE_EVENT,
  ...
  vr = VK_CALL(vkCreateSemaphore(chain->queue->device->vk_device, &create_info,
                                 NULL, &chain->present.vk_blit_semaphore));
  if (vr < 0) {
  ...
  vr =
      VK_CALL(vkCreateFence(chain->queue->device->vk_device, &fence_create_info,
                            NULL, &chain->present.vk_acquire_fence));
  if (vr < 0) {
...
static void dxgi_vk_swap_chain_drain_waiter(struct dxgi_vk_swap_chain *chain) {
  if (chain->wait_thread.active) {
    /* Waits until all swapchain waits have been processed. Required before we
...
static void dxgi_vk_swap_chain_wait_and_reset_acquire_fence(
  ...
  if (chain->present.acquire_fence_pending) {
     ...
     * There is little reason to add complexity with semaphores since behavior
     * is implementation defined regarding if AcquireNextImage is synchronous or
     * not. */
    vr = VK_CALL(vkWaitForFences(vk_device, 1, &chain->present.vk_acquire_fence,
                                 VK_TRUE, UINT64_MAX));
    if (vr < 0)
      ERR("Failed to wait for fence, vr %d\n", vr);
    VKD3D_DEVICE_REPORT_BREADCRUMB_IF(chain->queue->device,
...
static void dxgi_vk_swap_chain_destroy_swapchain_in_present_task(
    struct dxgi_vk_swap_chain *chain) {
  const struct vkd3d_vk_device_procs *vk_procs =
      &chain->queue->device->vk_procs;
  VkQueue vk_queue;
   ...
   * but when destroying vk_release_semaphore we might be in a state where we
   * submitted blit command buffer, but never waited on the semaphore in
   * vkQueuePresent, so we would still need this WaitIdle() most likely. */
  ...
  for (i = 0; i < ARRAY_SIZE(chain->present.vk_release_semaphores); i++)
    VK_CALL(vkDestroySemaphore(chain->queue->device->vk_device,
                               chain->present.vk_release_semaphores[i], NULL));
  memset(chain->present.vk_backbuffer_images, 0,
         sizeof(chain->present.vk_backbuffer_images));
  memset(chain->present.vk_backbuffer_image_views, 0,
         sizeof(chain->present.vk_backbuffer_image_views));
  memset(chain->present.vk_release_semaphores, 0,
         sizeof(chain->present.vk_release_semaphores));
...
static bool dxgi_vk_swap_chain_accept_format(const VkSurfaceFormatKHR *format,
                                             VkFormat vk_format) {
...
static bool dxgi_vk_swap_chain_find_surface_format(
    struct dxgi_vk_swap_chain *chain, VkFormat vk_format,
    VkColorSpaceKHR color_space, VkSurfaceFormatKHR *format) {
  uint32_t i;

  for (i = 0; i < chain->properties.format_count; i++) {
    if (dxgi_vk_swap_chain_accept_format(&chain->properties.formats[i],
                                         vk_format) &&
...
static bool dxgi_vk_swap_chain_select_format(struct dxgi_vk_swap_chain *chain,
                                             VkSurfaceFormatKHR *format) {
  VkFormat vk_format =
      vkd3d_get_format(chain->queue->device, chain->request.dxgi_format, false)
          ->vk_format;
  VkColorSpaceKHR vk_color_space =
      convert_color_space(chain->request.dxgi_color_space_type);

  if (dxgi_vk_swap_chain_find_surface_format(chain, vk_format, vk_color_space,
                                             format))
    return true;

  /* If we're using sRGB swapchains, we can fallback.
   * Usually happens for RGBA8 or 10-bit UNORM and display does not support it
   * as a present format. This can be trivially worked around by selecting e.g.
   * BGRA8. */
  if (vk_color_space == VK_COLOR_SPACE_SRGB_NONLINEAR_KHR) {
    return dxgi_vk_swap_chain_find_surface_format(chain, VK_FORMAT_UNDEFINED,
                                                  vk_color_space, format);
  } else {
...
static bool
dxgi_vk_swap_chain_check_present_mode_support(struct dxgi_vk_swap_chain *chain,
  ...
  VK_CALL(vkGetPhysicalDeviceSurfacePresentModesKHR(
      vk_physical_device, chain->vk_surface, &mode_count, supported_modes));
...
static void
dxgi_vk_swap_chain_init_blit_pipeline(struct dxgi_vk_swap_chain *chain) {
  struct d3d12_device *device = chain->queue->device;
  struct vkd3d_swapchain_pipeline_key key;
  HRESULT hr;

  key.bind_point = VK_PIPELINE_BIND_POINT_GRAPHICS;
  key.filter = chain->desc.Scaling == DXGI_SCALING_NONE ? VK_FILTER_NEAREST
                                                        : VK_FILTER_LINEAR;
  key.format = chain->present.backbuffer_format;

  if (FAILED(hr = vkd3d_meta_get_swapchain_pipeline(&device->meta_ops, &key,
                                                    &chain->present.pipeline)))
...
typedef struct proton_streamer_shm *(__stdcall *AllocateStreamerShm)();
typedef void(__stdcall *proton_streamer_signal_new_texture)(
    struct proton_streamer_shm *shm, int texture);
typedef void(__stdcall *proton_streamer_get_output_texture)(
    struct proton_streamer_shm *shm, int *out_texture);

AllocateStreamerShm allocate_fn = NULL;
...
static void dxgi_vk_swap_chain_recreate_swapchain_in_present_task(
  ...
  /* If we fail to query formats we are hosed, treat it as a SURFACE_LOST
   * scenario. */
  ...
  new_occlusion_state = surface_caps.maxImageExtent.width == 0 ||
                        surface_caps.maxImageExtent.height == 0;
  vkd3d_atomic_uint32_store_explicit(&chain->present.is_occlusion_state,
                                     (uint32_t)new_occlusion_state,
  ...
  /* Sanity check, this cannot happen on Win32 surfaces, but could happen on
   * Wayland. */
   ...
   * Where we have modified color space, but not yet changed user backbuffer
   * format. */
  ...
  if (!dxgi_vk_swap_chain_check_present_mode_support(chain, present_mode)) {
    if (present_mode == VK_PRESENT_MODE_IMMEDIATE_KHR &&
        dxgi_vk_swap_chain_check_present_mode_support(
            chain, VK_PRESENT_MODE_MAILBOX_KHR)) {
  ...
  /* We don't block directly on Present(), so there's no reason to use more than
   * 3 images if even application requests more. We could get away with 2 if we
   * used WSI acquire semaphore and async acquire was supported, but e.g. Mesa
   * does not support that. However, without present-wait, we'll have to inherit
   * quirky behavior from legacy swapchain and use minImageCount = BufferCount
   * + 1. */
  swapchain_create_info.minImageCount =
      max(max(chain->request.target_min_image_count, 3u),
  ...
  if (strlen(count_env) > 0) {
    ...
    swapchain_create_info.minImageCount =
        max(surface_caps.minImageCount, override_image_count);
  ...
  swapchain_create_info.imageExtent.width =
      max(swapchain_create_info.imageExtent.width,
          surface_caps.minImageExtent.width);
  swapchain_create_info.imageExtent.width =
      min(swapchain_create_info.imageExtent.width,
          surface_caps.maxImageExtent.width);
  swapchain_create_info.imageExtent.height =
      max(swapchain_create_info.imageExtent.height,
          surface_caps.minImageExtent.height);
  swapchain_create_info.imageExtent.height =
      min(swapchain_create_info.imageExtent.height,
          surface_caps.maxImageExtent.height);

  vr = VK_CALL(vkCreateSwapchainKHR(vk_device, &swapchain_create_info, NULL,
                                    &chain->present.vk_swapchain));
  if (vr < 0) {
  ...
  chain->present.backbuffer_count =
      ARRAY_SIZE(chain->present.vk_backbuffer_images);
  VK_CALL(vkGetSwapchainImagesKHR(vk_device, chain->present.vk_swapchain,
                                  &chain->present.backbuffer_count,
                                  chain->present.vk_backbuffer_images));
  ...
  if (!chain->present.vk_blit_command_pool) {
    ...
    command_pool_create_info.queueFamilyIndex =
        chain->queue->vkd3d_queue->vk_family_index;
...
static bool request_needs_swapchain_recreation(
    const struct dxgi_vk_swap_chain_present_request *request,
    const struct dxgi_vk_swap_chain_present_request *last_request) {
  return request->dxgi_color_space_type !=
             last_request->dxgi_color_space_type ||
         request->dxgi_format != last_request->dxgi_format ||
         request->target_min_image_count !=
             last_request->target_min_image_count ||
         (!!request->swap_interval) != (!!last_request->swap_interval);
}

static void dxgi_vk_swap_chain_present_signal_blit_semaphore(
    struct dxgi_vk_swap_chain *chain) {
  const struct vkd3d_vk_device_procs *vk_procs =
      &chain->queue->device->vk_procs;
...
static void
dxgi_vk_swap_chain_record_render_pass(struct dxgi_vk_swap_chain *chain,
  ...
  blank_present =
      vkd3d_atomic_uint32_load_explicit(&resource->initial_layout_transition,
                                        vkd3d_memory_order_relaxed) != 0;

  if (blank_present)
    WARN("Application is presenting user index %u, but it has never been "
         "rendered to.\n",
  ...
  attachment_info.imageView =
      chain->present.vk_backbuffer_image_views[swapchain_index];
  ...
  if (!blank_present) {
    ...
    image_info.imageView =
        chain->user.vk_image_views[chain->request.user_index];
    image_info.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    image_info.sampler = VK_NULL_HANDLE;

    VK_CALL(vkCmdPushDescriptorSetKHR(
        vk_cmd, VK_PIPELINE_BIND_POINT_GRAPHICS,
...
static void
dxgi_vk_swap_chain_create_streamer_texture(struct dxgi_vk_swap_chain *chain) {
  const struct vkd3d_vk_device_procs *vk_procs =
    &chain->queue->device->vk_procs;
...
static void
dxgi_vk_swap_chain_record_render_pass_streamer(struct dxgi_vk_swap_chain *chain,
                                               VkCommandBuffer vk_cmd,
                                               uint32_t swapchain_index) {
  const struct vkd3d_vk_device_procs *vk_procs =
      &chain->queue->device->vk_procs;
  ...
  attachment_info.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO_KHR;
  attachment_info.imageView =
      chain->present.vk_streamer_image_views[swapchain_index];
  attachment_info.imageLayout = VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL;
  attachment_info.loadOp = VK_ATTACHMENT_LOAD_OP_DONT_CARE;
  attachment_info.storeOp = VK_ATTACHMENT_STORE_OP_STORE;
  ...
  image_barrier.srcQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
  image_barrier.dstQueueFamilyIndex = VK_QUEUE_FAMILY_IGNORED;
  image_barrier.image = chain->present.vk_streamer_textures[swapchain_index];
  image_barrier.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
  image_barrier.subresourceRange.levelCount = 1;
  image_barrier.subresourceRange.layerCount = 1;
  ...
  if (!blank_present) {
    ...
    write_info.descriptorCount = 1;
    image_info.imageView =
        chain->user.vk_image_views
            [chain->request
                 .user_index]; // chain->present.vk_streamer_image_views[swapchain_index];
    image_info.imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;
    image_info.sampler = VK_NULL_HANDLE;
...
void dxgi_vk_swap_chain_streamer_worked(void *args_t) {
  struct dxgi_vk_swap_chain *chain;
  chain = args_t;
  ...
  for (;;) {
    ...
    while (!chain->streamer_thread.wait_queue_count)
      pthread_cond_wait(&chain->streamer_thread.cond,
                        &chain->streamer_thread.lock);
    texture_id = chain->streamer_thread.wait_queue[0].texture_idx;
    ...
    memmove(chain->streamer_thread.wait_queue,
            chain->streamer_thread.wait_queue + 1,
            chain->streamer_thread.wait_queue_count *
                sizeof(*chain->streamer_thread.wait_queue));
    if (chain->streamer_thread.wait_queue_count == 0)
      pthread_cond_signal(&chain->streamer_thread.cond);
    pthread_mutex_unlock(&chain->streamer_thread.lock);
  }
}

static bool dxgi_vk_swap_chain_submit_blit(struct dxgi_vk_swap_chain *chain,
                                           uint32_t swapchain_index) {
  const struct vkd3d_vk_device_procs *vk_procs =
      &chain->queue->device->vk_procs;
  VkDevice vk_device = chain->queue->device->vk_device;
  ...
  if (!chain->present.vk_release_semaphores[swapchain_index]) {
    ...
    semaphore_create_info.flags = 0;
    vr = VK_CALL(vkCreateSemaphore(
        vk_device, &semaphore_create_info, NULL,
        &chain->present.vk_release_semaphores[swapchain_index]));
    if (vr < 0) {
  ...
  if (!chain->present.vk_blit_command_buffers[swapchain_index]) {
    ...
    vr = VK_CALL(vkAllocateCommandBuffers(
        vk_device, &allocate_info,
  ...
  if (chain->present.vk_blit_fences[swapchain_index]) {
    vr = VK_CALL(vkWaitForFences(
        vk_device, 1, &chain->present.vk_blit_fences[swapchain_index], VK_TRUE,
        UINT64_MAX));
    VKD3D_DEVICE_REPORT_BREADCRUMB_IF(chain->queue->device,
                                      vr == VK_ERROR_DEVICE_LOST);
    if (vr < 0) {
    ...
    vr = VK_CALL(vkResetFences(
        vk_device, 1, &chain->present.vk_blit_fences[swapchain_index]));
    if (vr < 0) {
      ERR("Failed to reset fence, vr %d\n", vr);
      return false;
    }
    ...
    fence_create_info.flags = 0;
    vr =
        VK_CALL(vkCreateFence(vk_device, &fence_create_info, NULL,
                              &chain->present.vk_blit_fences[swapchain_index]));
    if (vr < 0) {
      ERR("Failed to create fence, vr %d.\n", vr);
      return false;
    }
  ...
  if(chain->present.vk_streamer_textures[0]) {
    ...
    if (streamer_tex != -1) {
      ...
      // signal_new_texture(shm, out_tex);

      if (!chain->streamer_thread.active) {
        chain->streamer_thread.active = true;
        vkd3d_array_reserve((void **)&chain->streamer_thread.wait_queue,
                            &chain->streamer_thread.wait_queue_size,
                            NUM_STREAMER_TEXTURES,
                            sizeof(*chain->streamer_thread.wait_queue));
        pthread_mutex_init(&chain->streamer_thread.lock, NULL);
        pthread_cond_init(&chain->streamer_thread.cond, NULL);

        /* Have to throw a thread under the bus unfortunately.
         * That thread will only wait on present IDs and release HANDLEs as
         * necessary. */
        if (pthread_create(&chain->streamer_thread.thread, NULL,
                           dxgi_vk_swap_chain_streamer_worked, chain)) {
          pthread_mutex_destroy(&chain->wait_thread.lock);
          pthread_cond_destroy(&chain->wait_thread.cond);
  ...
  memset(&signal_semaphore_info, 0, sizeof(signal_semaphore_info));
  signal_semaphore_info.sType = VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO;
  signal_semaphore_info.semaphore =
      chain->present.vk_release_semaphores[swapchain_index];
  signal_semaphore_info.stageMask = VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT;
  ...
  if (streamer_tex != -1) {
    struct streamer_thread_wait_entry *entry;
    pthread_mutex_lock(&chain->streamer_thread.lock);
    vkd3d_array_reserve((void **)&chain->streamer_thread.wait_queue,
                        &chain->streamer_thread.wait_queue_size,
                        chain->streamer_thread.wait_queue_count + 1,
...
static void dxgi_vk_swap_chain_present_recreate_swapchain_if_required(
    struct dxgi_vk_swap_chain *chain) {
...
static VkResult
dxgi_vk_swap_chain_try_acquire_next_image(struct dxgi_vk_swap_chain *chain) {
  const struct vkd3d_vk_device_procs *vk_procs =
      &chain->queue->device->vk_procs;
  VkDevice vk_device = chain->queue->device->vk_device;
  VkResult vr;
  ...
  vr = VK_CALL(vkAcquireNextImageKHR(vk_device, chain->present.vk_swapchain,
                                     UINT64_MAX, VK_NULL_HANDLE,
                                     chain->present.vk_acquire_fence,
...
static void
dxgi_vk_swap_chain_present_iteration(struct dxgi_vk_swap_chain *chain,
  ...
  /* Handle any errors and retry as needed. If we cannot make meaningful forward
   * progress, just give up and retry later. */
  ...
  if (vr == VK_ERROR_OUT_OF_DATE_KHR) {
    ...
    /* If the surface is lost, we cannot expect to get forward progress. Just
     * keep rendering to nothing. */
    chain->present.is_surface_lost = true;
  }
   ...
   * Non-FIFO swapchains will pump their frame latency handles through the
   * fallback path of blit command being done. Especially on Xwayland, the
   * present ID is updated when images actually hit on-screen due to MAILBOX
   * behavior. This would unnecessarily stall our progress. */
  if (chain->wait_thread.active && !chain->present.present_id_valid &&
      chain->request.swap_interval > 0) {
...
static void
dxgi_vk_swap_chain_signal_waitable_handle(struct dxgi_vk_swap_chain *chain) {
  HRESULT hr;
  ...
  if (chain->present.present_id_valid) {
    ...
    if (vkd3d_native_sync_handle_is_valid(
            chain->frame_latency_event_internal)) {
      if (FAILED(hr = d3d12_fence_set_native_sync_handle_on_completion(
                     impl_from_ID3D12Fence1(chain->present.frame_latency_fence),
                     chain->present.frame_latency_count,
...
static void dxgi_vk_swap_chain_present_callback(void *chain_) {
  ...
  next_request =
      &chain
           ->request_ring[next_present_count % ARRAY_SIZE(chain->request_ring)];
  ...
  /* If no QueuePresentKHRs successfully commits a present ID, we'll fallback to
   * a normal queue signal. */
  chain->present.present_id_valid = false;

  /* There is currently no present timing in Vulkan we can rely on, so just
   * duplicate blit them as needed. This happens on a thread, so the blocking
   * should not be a significant problem.
   * TODO: Propose VK_EXT_present_interval. */
  present_count = max(1u, chain->request.swap_interval);

  /* If we hit the legacy way of synchronizing with swapchain, blitting multiple
   * times would be horrible. */
  if (!chain->wait_thread.active)
    present_count = 1;

  for (i = 0; i < present_count; i++) {
    /* A present iteration may or may not render to backbuffer. We'll apply best
     * effort here. Forward progress must be ensured, so if we cannot get
     * anything on-screen in a reasonable amount of retries, ignore it. */
    dxgi_vk_swap_chain_present_iteration(chain, 0);
  }

  /* When this is signalled, lets main thread know that it's safe to free user
   * buffers. Signal this just once on the outside since we might have retries,
   * swap_interval > 1, etc, which complicates command buffer recycling. */
  ...
  vkd3d_atomic_uint32_store_explicit(&chain->present.present_count,
                                     next_present_count,
                                     vkd3d_memory_order_release);

  /* Considerations for implementations without KHR_present_wait which we
   * inherit from the legacy swapchain. To have any hope of achieving reasonable
   * and bounded latency, we need to use the method where we eagerly acquire
   * next image right after present. This avoids any late blocking when
   * presenting, which is a disaster for latency. The present caller must then
   * block on the queue reaching here. To avoid hard deadlocks with present
   * wait-before-signal, a reasonably small timeout is possible. A deadlock in
   * this scenario has only been observed on NVIDIA, which supports present_wait
   * anyways.
   *
   * On implementations with present wait, blocking late is a good idea since it
   * avoids potential GPU bubbles. While blocking here, we cannot submit more
   * work to the GPU on this queue, since we're in a queue callback. */
  if (!chain->wait_thread.active) {
    dxgi_vk_swap_chain_try_acquire_next_image(chain);
    /* The old implementation used acquire semaphores, and did not block, so to
     * maintain same behavior, defer blocking on the VkFence. */
    if (vkd3d_native_sync_handle_is_valid(chain->present_request_done_event))
      vkd3d_native_sync_handle_signal(chain->present_request_done_event);
  }

#ifdef VKD3D_ENABLE_BREADCRUMBS
  vkd3d_breadcrumb_tracer_update_barrier_hashes(
      &chain->queue->device->breadcrumb_tracer);
#endif
}
...
static void *dxgi_vk_swap_chain_wait_worker(void *chain_) {
  ...
  for (;;) {
    ...
    VK_CALL(vkWaitForPresentKHR(chain->queue->device->vk_device,
                                chain->present.vk_swapchain, next_wait_id,
                                UINT64_MAX));
    ...
    if (vkd3d_native_sync_handle_is_valid(chain->frame_latency_event)) {
      if ((previous_semaphore = vkd3d_native_sync_handle_release(
               chain->frame_latency_event, 1)) >= 0) {
        if (previous_semaphore >= (int)chain->frame_latency) {
          WARN("Incrementing frame latency semaphore beyond max latency. "
               "Did application forget to acquire? (new count = %d, max "
               "latency = %u)\n",
               previous_semaphore + 1, chain->frame_latency);
        }
      } else
        WARN("Failed to increment swapchain semaphore. Did application forget "
             "to acquire?\n");
    }
    ...
    memmove(chain->wait_thread.wait_queue, chain->wait_thread.wait_queue + 1,
            chain->wait_thread.wait_queue_count *
                sizeof(*chain->wait_thread.wait_queue));
...
static HRESULT
dxgi_vk_swap_chain_init_waiter_thread(struct dxgi_vk_swap_chain *chain) {
  char env[8];
   ...
   */
  if (pthread_create(&chain->wait_thread.thread, NULL,
                     dxgi_vk_swap_chain_wait_worker, chain)) {
    pthread_mutex_destroy(&chain->wait_thread.lock);
    pthread_cond_destroy(&chain->wait_thread.cond);
    return E_OUTOFMEMORY;
  }

  if (vkd3d_get_env_var("VKD3D_SWAPCHAIN_DEBUG_LATENCY", env, sizeof(env)) &&
      strcmp(env, "1") == 0)
...
static HRESULT dxgi_vk_swap_chain_init(struct dxgi_vk_swap_chain *chain,
                                       IDXGIVkSurfaceFactory *pFactory,
                                       const DXGI_SWAP_CHAIN_DESC1 *pDesc,
                                       struct d3d12_command_queue *queue) {
  HRESULT hr;
  ...
  INFO("Creating swapchain (%u x %u), BufferCount = %u.\n", pDesc->Width,
       pDesc->Height, pDesc->BufferCount);
...
static HRESULT STDMETHODCALLTYPE dxgi_vk_swap_chain_factory_CreateSwapChain(
    IDXGIVkSwapChainFactory *iface, IDXGIVkSurfaceFactory *pFactory,
    const DXGI_SWAP_CHAIN_DESC1 *pDesc, IDXGIVkSwapChain **ppSwapchain) {
  struct dxgi_vk_swap_chain_factory *factory =
      impl_from_IDXGIVkSwapChainFactory(iface);
  struct dxgi_vk_swap_chain *chain;
  HRESULT hr;
  ...
  if (FAILED(hr = dxgi_vk_swap_chain_init(chain, pFactory, pDesc,
                                          factory->queue))) {
    vkd3d_free(chain);
    return hr;
  }
...
static CONST_VTBL struct IDXGIVkSwapChainFactoryVtbl
    dxgi_vk_swap_chain_factory_vtbl = {
        /* IUnknown methods */
...
HRESULT
dxgi_vk_swap_chain_factory_init(struct d3d12_command_queue *queue,
                                struct dxgi_vk_swap_chain_factory *chain) {
  chain->IDXGIVkSwapChainFactory_iface.lpVtbl =
      &dxgi_vk_swap_chain_factory_vtbl;
  chain->queue = queue;
  return S_OK;
}
